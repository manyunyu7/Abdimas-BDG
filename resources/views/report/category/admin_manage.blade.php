...
@section('page-wrapper')
    ...
    <div class="modal fade" id="destroy-modal" tabindex="-1" role="dialog" aria-labelledby="destroy-modalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="destroy-modalLabel">Apakah Anda Yakin Ingin Menghapus Category Ini ??</h5>
    ...
    <script type="text/javascript">
        $(function() {
            var table = $('#table_data').DataTable({
                ...
                ajax: {
                    type: "get",
                    url: "{{ url('report_category/manage') }}",
                    data: {
                        "_token": "{{ csrf_token() }}",
                    },
                    async: true,
        ...
        $(".btn-destroy").on("click", function() {
            ...
            console.log(id);
            $.ajax({
                url: "{{ URL::to('/') }}/report_category/" + id + "/delete",
                data: {
                    "_token": "{{ csrf_token() }}",
                    "id": id,
                },
                ...
                success: function(response) {
                    ...
                    swal("Sukses", "Berhasil Menghapus Data Berita ", "success");
                },
                error: function(xhr, error, code) {
                    var err = eval("(" + xhr.responseText + ")");
                    console.log(error);
                    console.log(err);
                    swal("Error", "Gagal Menghapus Data Berita ", "error");
                }
            });
        })
